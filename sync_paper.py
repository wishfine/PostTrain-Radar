...
def generate_markdown(meta, reading_notes=None, judgment=None, draft_review=None,
    ...
    notes_content = reading_notes if reading_notes else """> 说明：这里是人工精读笔记区，任何自动同步均不会覆盖此区。
*   **阅读时间**: 
*   **精读笔记**: 
    *   (在此记录您的阅读细节、推导过程、关键公式或模型架构 of 独特理解)"""

    judgment_content = judgment if judgment else """> 说明：这里是人工独立技术判断区，任何自动同步均不会覆盖此区。
*   **论文盲点/局限性**: 
*   **实验设计局限**: 
*   **我的评价**: 
    *   (在这里写下您对该文的真实技术评价，是否真正解决了痛点？)"""

    review_content = draft_review if draft_review else """> 说明：人工对 AI 生成草稿的审查记录，自动更新不会覆盖。
*   **AI Draft 是否可信**: High / Medium / Low
*   **错误点**: 
*   **我修正后的理解**: 
*   **是否需要重新生成**: """

    summary_val = meta.get("summary") or meta.get("abstract") or ""
    summary_val = str(summary_val).strip()
    if summary_val and summary_val not in ("...", "") and len(summary_val) > 5:
        clean_sum = " ".join(summary_val.split())
        if len(clean_sum) > 300:
            problem_solved = f"{clean_sum[:300]}..."
        else:
            problem_solved = clean_sum
    else:
        problem_solved = "待精读后补充"

    ai_draft_content = ai_draft if ai_draft else f"""*   **一句话总结**: 作者认为 ______，因此提出 ______。
*   **解决的问题**: {problem_solved}
*   **核心方法**: 
*   **实验结论**: """

    knowledge_ext_content = knowledge_ext if knowledge_ext else """*   **可提炼的方法/技术路线**: ➔ 待人工补充
*   **可引入的问题意识/技术冲突**: ➔ 待人工补充
*   **有启发的后续实验设计**: """
...
*   **分享目标受众**: """
    ...
    next_action_content = next_action if next_action else """*   [ ] 读 Introduction
*   [ ] 读 Method
*   [ ] 读 Experiments
*   [ ] 看 Ablation
*   [ ] 找相关论文对比
...
*   [ ] 判断是否生成分享稿"""
    ...
    metadata_rows = [
        ...
        f"| Data Origin | {meta.get('data_origin', 'Unknown')} |",
        f"| Type | #Other |",
        f"| Tags | #Unread |",
        f"| Priority | Medium |",
        f"| Relevance Level | D_Irrelevant |",
        f"| Confidence | 0.0 |",
        f"| Reason | No reason provided. |",
        f"| Method Tags |  |",
        f"| Problem Tags |  |",
        f"| Core Post-Training | No |",
        f"| URL | [Abstract URL]({meta.get('abs_url', '')}) |",
        f"| PDF | [PDF URL]({meta.get('pdf_url', '')}) |"
    ]
    metadata_table = (
        "| Metadata Key | Value |\n"
        "| :--- | :--- |\n" +
